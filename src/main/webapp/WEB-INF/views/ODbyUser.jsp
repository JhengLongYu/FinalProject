...
let memberId = "${mbid}";
	...
	function processDetailsData(jsonString) {
		...
		segment += "<thead><tr><th colspan='7'>訂單資料明細</th></tr>";
		segment += "<tr><th>訂單號碼</th><th>商品圖片</th><th>產品編號</th><th>產品敘述</th><th>單價</th><th>購買數量</th></tr></thead><tbody>";
		$(document).ready(function() {
		    $('#example').DataTable();
		} );
		...
		for (let n = 0; n < shoppingCarts.length; n++) {
			...
			segment += "<td><img style= 'width:105px ;height:105px;' "+"src='" + imageURL +"?id="
			+ shoppingCart.itemId + "'></td>"
			segment += "<td>" + shoppingCart.itemId + "</td>";
			segment += "<td>" + shoppingCart.description + "</td>";
			segment += "<td>" + shoppingCart.unitPrice + "</td>";
			segment += "<td>" + shoppingCart.qty + "</td>";
			segment += "</tr>";
		}
		segment += "</tbody></table>";
		return segment;
